import { $ as addRoundedRectPath, J as QUARTER_PI, K as HALF_PI, X as TAU, Y as RAD_PER_DEG, Z as TWO_THIRDS_PI, _t as valueOrDefault, at as getAngleFromPoint, ct as isNumber, dt as toDegrees, et as callback, ft as toFont, gt as unclipArea, ht as toTRBLCorners, it as distanceBetweenPoints, lt as isNumberFinite, m as Element, mt as toRadians, n as Animations, nt as defaults, ot as isArray, p as DoughnutController, pt as toPadding, q as PI, rt as defined, st as isFunction, tt as clipArea, u as Chart, ut as isObject } from "./chart-olKhbaW_.js";
//#region node_modules/chartjs-plugin-annotation/dist/chartjs-plugin-annotation.esm.js
/*!
...
function getElements(visibleElements, event, options) {
	return (interaction.modes[options.mode] || interaction.modes.nearest)(visibleElements, event, options);
}
...
function getNearestItem(visibleElements, event, options) {
	...
	return filterElements(visibleElements, event, options).reduce((nearestItems, element) => {
		const distance = distanceBetweenPoints(event, getPointByAxis(event, element.getCenterPoint(), options.axis));
...
function inLabelRange(point, { rect, center }, axis, { rotation, borderWidth, hitTolerance }) {
	return inBoxRange(rotated(point, center, toRadians(-rotation)), rect, axis, {
		borderWidth,
		hitTolerance
	});
}
...
function toFonts(options, fitRatio) {
	...
	if (shouldFit(options, fitRatio)) return fonts.map(function(f) {
		const font = toFont(f);
...
function loadHooks(options, hooks, hooksContainer) {
	...
	hooks.forEach((hook) => {
		...
		} else if (defined(hooksContainer[hook])) delete hooksContainer[hook];
	});
	return activated;
}
function applyDefault(properties, options) {
	return defaultInitAnimation[options.type || "line"](properties);
}
...
function drawCallout(ctx, element) {
	...
	ctx.save();
	ctx.beginPath();
	if (!setBorderStyle(ctx, callout)) return ctx.restore();
...
function getDimensionByScale(scale, options) {
	if (scale) {
		...
		return {
			start: scaleValue(scale, options.min, reverse ? options.end : options.start),
			end: scaleValue(scale, options.max, reverse ? options.start : options.end)
		};
	}
}
/**
...
function updateListeners(chart, state, options) {
	...
	moveHooks.forEach((hook) => {
		if (isFunction(options[hook])) state.moveListened = true;
	});
	if (!state.listened || !state.moveListened) state.annotations.forEach((scope) => {
		if (!state.listened && isFunction(scope.click)) state.listened = true;
...
function handleEvent(state, event, options) {
	if (state.listened) switch (event.type) {
		case "mousemove":
		case "mouseout": return handleMoveEvents(state, event, options);
		case "click": return handleClickEvents(state, event, options);
	}
}
function handleMoveEvents(state, event, options) {
	if (!state.moveListened) return;
	let elements;
	if (event.type === "mousemove") elements = getElements(state.visibleElements, event, options.interaction);
	else elements = [];
...
function updateHooks(chart, state, options) {
	...
	if (!state.hooked) visibleElements.forEach((scope) => {
		if (!state.hooked) elementHooks.forEach((hook) => {
			if (isFunction(scope.options[hook])) state.hooked = true;
		});
	});
}
/**
...
function invokeHook(state, element, hook) {
	if (state.hooked) return callback(element.options[hook] || state.hooks[hook], [element.$context]);
}
/**
* @typedef { import("chart.js").Chart } Chart
* @typedef { import("chart.js").Scale } Scale
...
function adjustScaleRange(chart, scale, annotations) {
	...
	if (changed && isFunction(scale.handleTickRangeOptions)) scale.handleTickRangeOptions();
}
/**
* @param {CoreAnnotationOptions[]} annotations
* @param {{ [key: string]: Scale }} scales
...
function verifyProperties(annotation, key) {
	...
	for (const prop of [
		...
		"Max",
		"Value"
	]) if (defined(annotation[axis + prop])) return true;
	return false;
}
function getScaleLimits(scales, scale, annotations) {
	...
	for (const annotation of annotations) if (annotation.scaleID === scaleID) updateLimits(annotation, scale, ["value", "endValue"], limits);
...
BoxAnnotation.descriptors = { label: { _fallback: true } };
var DoughnutLabelAnnotation = class extends Element {
	...
	resolveElementProperties(chart, options) {
		...
		if (shouldFit(options, _fitRatio)) labelSize = {
...
function getDatasetMeta(chart, options) {
	return chart.getSortedVisibleDatasetMetas().reduce(function(result, value) {
		...
		if (controller instanceof DoughnutController && isControllerVisible(chart, options, value.data) && (!result || controller.innerRadius < result.controller.innerRadius) && controller.options.circumference >= 90) return value;
		return result;
	}, void 0);
}
function isControllerVisible(chart, options, elements) {
	if (!options.autoHide) return true;
	for (let i = 0; i < elements.length; i++) if (!elements[i].hidden && chart.getDataVisibility(i)) return true;
}
function getControllerMeta({ chartArea }, options, meta) {
	...
	return {
		controllerMeta: {
			...
			_radius,
			_counterclockwise,
			...getAngles(_counterclockwise ? top + space : bottom - space, x, y, _radius)
		},
		point,
...
LabelAnnotation.defaultRoutes = { borderColor: "color" };
...
var sqr = (v) => v * v;
var rangeLimit = (mouseX, mouseY, { x, y, x2, y2 }, axis) => axis === "y" ? {
	start: Math.min(y, y2),
	end: Math.max(y, y2),
	value: mouseY
} : {
...
var angleInCurve = (start, cp, end, t) => -Math.atan2(coordAngleInCurve(start.x, cp.x, end.x, t), coordAngleInCurve(start.y, cp.y, end.y, t)) + .5 * PI;
var LineAnnotation = class extends Element {
	...
	draw(ctx) {
		...
		ctx.save();
		if (!setBorderStyle(ctx, options)) return ctx.restore();
		setShadowStyle(ctx, options);
	...
	resolveElementProperties(chart, options) {
		...
		properties.initProperties = initAnimationProperties(chart, properties, options);
		if (options.curve) properties.cp = getControlPoint(properties, options, distanceBetweenPoints({
			x: properties.x,
			y: properties.y
		}, {
			x: properties.x2,
...
LineAnnotation.descriptors = { arrowHeads: {
	start: { _fallback: true },
	end: { _fallback: true },
	_fallback: true
} };
LineAnnotation.defaultRoutes = { borderColor: "color" };
function inAxisRange(element, { mouseX, mouseY }, axis, { hitSize, useFinalPosition }) {
	return inLimit(rangeLimit(mouseX, mouseY, element.getProps([
		"x",
		"y",
		"x2",
		"y2"
	], useFinalPosition), axis), hitSize) || isOnLabel(element, {
		mouseX,
		mouseY
...
function resolveLabelElementProperties(chart, properties, options) {
	...
	return calculateLabelPosition(properties, options, {
		width: textSize.width + padding.width + borderWidth,
		height: textSize.height + padding.height + borderWidth,
		padding
	}, chart.chartArea);
}
...
EllipseAnnotation.descriptors = { label: { _fallback: true } };
...
var PolygonAnnotation = class extends Element {
	inRange(mouseX, mouseY, axis, useFinalPosition) {
		if (axis !== "x" && axis !== "y") return this.options.radius >= .1 && this.elements.length > 1 && pointIsInPolygon(this.elements, mouseX, mouseY, useFinalPosition);
		const rotatedPoint = rotated({
			x: mouseX,
			y: mouseY
		}, this.getCenterPoint(useFinalPosition), toRadians(-this.options.rotation));
...
Object.keys(annotationTypes).forEach((key) => {
	defaults.describe(`elements.${annotationTypes[key].id}`, { _fallback: "plugins.annotation.common" });
});
...
function updateElements(chart, state, options, mode) {
	...
	for (let i = 0; i < annotations.length; i++) {
		...
		if (!defined(element.x)) Object.assign(element, properties);
...
function updateSubElements(mainElement, elements, resolver, animations) {
	...
	for (let i = 0; i < elements.length; i++) {
		...
		properties.options = resolveAnnotationOptions(resolver[definition.optionScope].override(definition));
...
function resolveObj(resolver, defs) {
	...
	for (const prop of Object.keys(defs)) {
		...
		if (isIndexable(prop) && isArray(value)) result[prop] = value.map((item) => resolve(item, optDefs));
...
var annotation = {
	...
	beforeUpdate(chart, args, options) {
		...
		else if (isArray(annotationOptions)) annotations.push(...annotationOptions);
	...
	beforeEvent(chart, args, options) {
		if (handleEvent(chartStates.get(chart), args.event, options)) args.changed = true;
	},
	afterDestroy(chart) {
...
function draw(chart, caller, clip) {
	...
	const state = chartStates.get(chart);
	if (clip) clipArea(ctx, chartArea);
...
function getDrawableElements(elements, caller) {
	...
	for (const el of elements) {
		...
		if (el.elements && el.elements.length) {
			for (const sub of el.elements) if (sub.options.display && sub.options.drawTime === caller) drawableElements.push({ element: sub });
		}
	}
